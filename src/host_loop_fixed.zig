//! Continuous host sense loop — sample plant → inject → tick (OS-like body cycle).
//! Body daemon: longer run with optional sleep between cycles.
//! Still Zig-only; mind remains Fixed lattice authority.

const std = @import("std");
const fixed = @import("fixed.zig");
const host_f = @import("host_senses_fixed.zig");
const organism_f = @import("organism_fixed.zig");
const Fixed = fixed.Fixed;

pub const HostLoopReport = struct {
    ok: bool,
    n_ticks: u32,
    n_live_display: u32,
    n_live_mic: u32,
    spikes: u32,
    episodes: u32,
    spoke: bool,
    sleep_ms: u32,
};

pub const HostLoopConfig = struct {
    n_ticks: u32 = 24,
    do_speak: bool = true,
    /// wall sleep between body cycles (daemon pace); 0 = as-fast-as-possible
    sleep_ms: u32 = 0,
    /// print progress every N ticks (0 = silent)
    report_every: u32 = 0,
};

/// Run N body cycles: host sample → bio inject → organism tick.
pub fn runHostLoop(n_ticks: u32, do_speak: bool) HostLoopReport {
    return runHostLoopCfg(.{ .n_ticks = n_ticks, .do_speak = do_speak });
}

pub fn runHostLoopCfg(cfg: HostLoopConfig) HostLoopReport {
    var org = organism_f.OrganismF.init();
    org.encode_every = 6;
    org.steps_per_tick = 2;
    if (cfg.do_speak) org.speak_every = 5;

    var n_disp: u32 = 0;
    var n_mic: u32 = 0;
    var t: u32 = 0;
    while (t < cfg.n_ticks) : (t += 1) {
        var sample: host_f.HostSample = .{};
        host_f.sampleHost(&sample);
        if (sample.live_display) n_disp += 1;
        if (sample.live_mic) n_mic += 1;
        host_f.injectIntoOrganism(&org, &sample);
        if (cfg.do_speak) {
            org.setMeaning(sample.vision[0..]);
        }
        const st = org.tickOnce();
        if (cfg.report_every > 0 and ((t + 1) % cfg.report_every) == 0) {
            std.debug.print(
                "body t={d}/{d} spikes_win={d} eps={d} live_d={} live_m={}\n",
                .{ t + 1, cfg.n_ticks, st.spikes, st.episodes, sample.live_display, sample.live_mic },
            );
        }
        if (cfg.sleep_ms > 0) {
            std.Thread.sleep(@as(u64, cfg.sleep_ms) * std.time.ns_per_ms);
        }
    }

    const ok = cfg.n_ticks >= 1 and (n_disp + n_mic > 0 or org.store.n >= 1 or org.brain.totalSpikes() >= 1);
    return .{
        .ok = ok,
        .n_ticks = cfg.n_ticks,
        .n_live_display = n_disp,
        .n_live_mic = n_mic,
        .spikes = org.brain.totalSpikes(),
        .episodes = @intCast(org.store.n),
        .spoke = cfg.do_speak,
        .sleep_ms = cfg.sleep_ms,
    };
}

/// Body daemon default: longer continuous run (boot-friendly).
pub fn runBodyDaemon() HostLoopReport {
    return runHostLoopCfg(.{
        .n_ticks = 120,
        .do_speak = true,
        .sleep_ms = 20,
        .report_every = 20,
    });
}
